...
# asdf
if [ ! -d "~/.asdf" ]; then
    printf "\n%s asdf" "${installing_text}"
    git clone https://github.com/asdf-vm/asdf.git ~/.asdf
    git checkout "$(git describe --abbrev=0 --tags)"
    printf "\n%s asdf" "${successful_text}"
    printf "\nℹ️  Shell must be restarted before asdf is available on your PATH. Re-run this script."
    exit 0
fi

# nodejs
printf "\n%s NodeJS" "${installing_text}"
printf "\n%s default-npm-packages" "${symlink_text}"
...
printf "\n%s NodeJS" "${successful_text}"

# Python
printf "\n%s Python" "${installing_text}"
case "${osType}" in
Linux*)
    sudo apt-get update
